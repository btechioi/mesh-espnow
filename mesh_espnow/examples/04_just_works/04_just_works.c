 ...
 *   ┌─ Boards discover each other via ESP-NOW beacons
 *   ├─ One node is elected root (highest capability + uptime)
 *   ├─ All nodes report simulated sensor data to the root
 *   ├─ Root prints every reading it receives
 *   └─ If root disappears → instant re-election
 *
 * No configuration. No gateway mode. No manual setup.
 *
 * Works with:
 *   - Arduino IDE   (include from .ino file)
 *   - ESP-IDF       (compile directly as main source)
...
#define MSG_SENSOR      0x03   /* "Here is a sensor reading" */
...
static uint32_t    s_root_last_ms;
...
static void elect(void) {
    ...
    if (best_id != s_root_id) {
        ...
        s_root_id = best_id;
        s_root_last_ms = TIME_MS();
        if (old) {
            ESP_LOGI(TAG, "Root: 0x%08X (score=%u) was 0x%08X", best_id, best_scr, old);
        } else {
            ESP_LOGI(TAG, "Root: 0x%08X (score=%u)", best_id, best_scr);
        }
...
static void send_sensor(void) {
    uint32_t id = mesh_espnow_get_node_id();
    s_temp += ((float)(esp_random() % 100) - 50.0f) / 100.0f;
    s_hum  += ((float)(esp_random() % 100) - 50.0f) / 50.0f;
    if (s_temp > 35.0f) s_temp = 35.0f;
    if (s_temp < 10.0f) s_temp = 10.0f;
    if (s_hum  > 90.0f) s_hum  = 90.0f;
    if (s_hum  < 20.0f) s_hum  = 20.0f;
    ...
    memcpy(buf + n, &s_temp,       4); n += 4;
    memcpy(buf + n, &s_hum,        4); n += 4;
    memcpy(buf + n, &batt,         4); n += 4;

    if (s_root_id && s_root_id != id) {
        esp_err_t e = mesh_espnow_send(s_root_id, buf, n, NULL);
        if (e == ESP_OK) {
            ESP_LOGI(TAG, "Data #%u -> root 0x%08X  temp=%.1f hum=%.1f",
                     s_sensor_seq, s_root_id, s_temp, s_hum);
        }
    } else if (s_im_root) {
        ESP_LOGI(TAG, "Data #%u (self)  temp=%.1f hum=%.1f", s_sensor_seq, s_temp, s_hum);
    }
    s_sensor_seq++;
}
...
static void handle_msg(uint32_t src, const uint8_t *d, uint16_t len) {
    ...
    case MSG_CANDIDACY: {
        ...
        upsert_cand(id, scr, caps, up);

        if (!s_im_root) {
            uint16_t my = my_score();
            if (my > scr) send_candidacy();
        }
        elect();
        break;
    }
    ...
    case MSG_SENSOR: {
        ...
        memcpy(&hum,  d + 13, 4);
        memcpy(&batt, d + 17, 4);
        ESP_LOGI(TAG, "<<< 0x%08X [#%u] temp=%.1fC hum=%.1f%% batt=%umV",
...
static void cb_data(uint32_t src, const uint8_t *d, uint16_t len, int8_t rssi) {
...
static void cb_discover(uint32_t id, int8_t rssi) {
    ESP_LOGI(TAG, "+ 0x%08X (RSSI=%d)", id, rssi);
    send_candidacy();
}
...
static void cb_route(uint32_t dest, uint32_t next, uint8_t hops) {
    ESP_LOGD(TAG, "Route 0x%08X -> 0x%08X (%u hops)", dest, next, hops);
}
...
void setup(void) {
    ...
    ESP_LOGI(TAG, "Waiting for other nodes...");
    ESP_LOGI(TAG, "");
}
...
void loop(void) {
    ...
    if (s_im_root && now - s_root_last_ms >= HEARTBEAT_IVL_MS) {
        send_heartbeat();
        s_root_last_ms = now;
    }
    ...
    if (now - s_diag_last_ms >= DIAG_IVL_MS) {
        ...
        if (mesh_espnow_get_stats(&st) == ESP_OK) {
            ...
            ESP_LOGI(TAG, "State: %s  Neighbors: %u  Routes: %u",
                     mesh_espnow_state_str(mesh_espnow_get_state()),
                     st.neighbor_count, st.route_count);
            ESP_LOGI(TAG, "TX: %u  RX: %u  Fwd: %u  Drop: %u  Retx: %u",
                     st.tx_packets, st.rx_packets,
                     st.forwarded, st.dropped, st.retransmissions);
            ESP_LOGI(TAG, "Root: 0x%08X  I_am_root: %s  Candidates: %d",
                     s_root_id, s_im_root ? "YES" : "no", s_cand_cnt);
        }
        if (s_cand_cnt > 0) {
            for (int i = 0; i < s_cand_cnt && i < 5; i++) {
                ESP_LOGI(TAG, "  %d. 0x%08X  score=%u  caps=0x%02X",
                         i + 1, s_cands[i].node_id,
                         s_cands[i].score, s_cands[i].caps);
            }
            if (s_cand_cnt > 5)
                ESP_LOGI(TAG, "  ... and %d more", s_cand_cnt - 5);
        }
        s_diag_last_ms = now;
    }
